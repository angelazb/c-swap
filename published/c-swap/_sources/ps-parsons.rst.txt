...
.. parsonsprob:: cs_swap_comments_pp
   :numbered: left
   :practice: T
   :adaptive:
   :noindent:

   The following has the correct code to ‘swap’ the string values in x and y (so that x ends up with y’s initial string value and y ends up with x’s initial string value), but the code is mixed up and contains one extra block which is not needed in a correct solution. Drag the needed blocks from the left into the correct order on the right. Check your solution by clicking on the Check button. You will be told if any of the blocks are in the wrong order or if you need to remove one or more blocks. After three incorrect attempts you will be able to use the Help Me button to make the problem easier.
   -----
...
.. parsonsprob:: cs_swap_code_and_comments_pp
   :numbered: left
   :practice: T
   :adaptive:
   :noindent:
...
.. parsonsprob:: cs_swap_code_only_pp
   :numbered: left
   :practice: T
   :adaptive:
   :noindent:
   ...
   return 0;
   =====
   for (int i = 0; i < len(x); i++) { #distractor: python function of length
   =====
   for (int i = 0; i < len(y); i++) { #distractor: python function of length
   =====
   for (int i = 0; i < len(temp); i++) { #distractor: python function of length

Feedback
==================================

.. shortanswer:: cs-parsons-sa
